...
use App\libs\protobuf\encoding\pb_base128;
use App\libs\protobuf\type\pb_scalar;
use App\libs\protobuf\type\pb_enum;
use App\libs\protobuf\type\pb_bytes;
use App\libs\protobuf\type\pb_string;
use App\libs\protobuf\type\Pbint;
use App\libs\protobuf\type\pb_bool;
use App\libs\protobuf\type\pb_signed_int;
use App\libs\protobuf\type\Pbstring;
use App\libs\protobuf\reader\pb_input_reader;
use App\libs\protobuf\reader\pb_input_string_reader;
...
abstract class PB_Message{
    ...
    public function __construct($reader=null){
        $this->reader = $reader;
        $this->value = $this;
        $this->base128 = new pb_base128(PB_Message::MODUS);
    }
    ...
    public function SerializeToString($rec=-1){
        ...
        if ($this->wired_type == PB_Message::WIRED_LENGTH_DELIMITED && $rec > -1)
        {
            $stringinner = $this->base128->set_value(strlen($stringinner) / PB_Message::MODUS) . $stringinner;
        }
    ...
    private function _ParseFromArray($length=99999999){
        ...
        while ($this->reader->get_pointer() - $_begin < $length)
        {
            ...
            if (!isset($this->fields[$messtypes['field']]))
            {
                ...
                if ($messtypes['wired'] == PB_Message::WIRED_LENGTH_DELIMITED)
                {
                    $consume = new PBString($this->reader);
                }
                else if ($messtypes['wired'] == PB_Message::WIRED_VARINT)
                {
                    $consume = new PB_Int($this->reader);
                }
                else
                {
    ...
    protected function _set_value($index, $value){
        ...
        else
        {
        	...
        	$class = "App\libs\protobuf\\type\\$fields";
